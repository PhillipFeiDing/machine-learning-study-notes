...
class LogisticRegression:
    ...
    def fit(self, X_train, y_train, eta=0.01, n_iters=1e4):
        ...
        def dJ(theta, X_b, y):
            diff = self._sigmoid(X_b.dot(theta)) - y
            temp = X_b.T.dot(diff)
            return temp / len(y)
    ...
    def _predict_probability(self, X_predict):
    ...
    def predict(self, X_predict):
        ...
        prob = self._predict_probability(X_predict)
    ...
    def __repr__(self):
        return "LogisticRegression(intercept=%.3f, coeff=%s)" % (self.intercept_, str(np.round(self.coeff_, 3)))
